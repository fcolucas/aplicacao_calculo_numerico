    ...
    while(menu){
        ...
        switch(op){
            case 'C':
                ...
                char s[50]; //String auxiliar para fazer a valida��o do n�mero.
				scanf("%s", s);
                fflush(stdin);
                if((strstr(s, ",") != NULL) || ((strstr(s, ",") == NULL) && (strstr(s, ".") == NULL))){ //Verifica se o valor digitado pelo usu�rio tem uma v�rgula, ou se n�o cont�m uma virgular nem um ponto ou um n�o cont�m espa�o em branco. Caso contr�rio, devolver� um erro.
                	sscanf(s, "%lf", &num);
					conversao(num);	
				}else{
